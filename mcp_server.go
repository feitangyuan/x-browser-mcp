...
func InitMCPServer(service *SearchService) *mcp.Server {
	...
	mcp.AddTool(server,
		&mcp.Tool{
			...
			Description: "Check whether the local X session is ready for search",
		},
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "start_login",
			Description: "Open a visible browser window so the user can log into X manually",
		},
	...
	mcp.AddTool(server,
		&mcp.Tool{
			...
			Description: "Read the current X home timeline from the local logged-in session and summarize what people are discussing",
		},
	...
	mcp.AddTool(server,
		&mcp.Tool{
			Name:        "search_x",
			Description: "Search recent X discussions using the local logged-in session and return summary plus representative quotes",
		},
